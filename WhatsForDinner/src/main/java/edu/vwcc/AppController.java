...
public class AppController {
    ...
    @PostMapping("/search")
    public String search(@RequestParam(required = false) String tag,
                         @RequestParam(required = false) String proteinsInput,
        ...
        List<String> proteins = recipeService.parseList(proteinsInput)
                .stream()
                .map(recipeService::normalizeProtein)
                .toList();

        List<String> ingredients = recipeService.parseList(ingredientsInput);

        List<Recipe> results =
                recipeService.searchByIngredientsAndProteins(ingredients, proteins);
